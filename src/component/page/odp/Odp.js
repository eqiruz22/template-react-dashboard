...
import { getOdp } from '../../../request/http'
...
const Odp = () => {
    ...
    useEffect(() => {
        ...
        if (storedUser) {
            ...
            if (user && user.token) {
                getOdp(setOdp, user, keyword, page, limit, setPage, setLimit, setRows, setPages)
            } else {
                navigate('/login');
            }
        } else {
                    ...
                    <div className='container-fluid'>
                        <div className='px-5'>
                            <div className="d-sm-flex align-items-center justify-content-between mb-4">
                                <h1 className="h3 mb-0 text-gray-800">Data ODP</h1>
                                ...
                                <div className='dropdown'>
                                    <Link to='/odp/create' className="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i
                                        className="fas fa-plus fa-sm text-white-50"></i> Create ODP</Link>
                            ...
                            <table className='table table-hover'>
                                <thead>
                                    <tr>
                                        ...
                                        <th scope='colSpan'>ODP Uplink</th>
                                        <th scope='colSpan'>ODP Port</th>
                                        <th scope='colSpan'>Capacity</th>
                                        <th scope='colSpan'>Location</th>
                                ...
                                {odp.length > 0 ?
                                    odp.map((item, index) =>
                                        <tbody key={`odp_${item.odp_id}`}>
                                            <tr key={`tr-odp_${index + 10}`}>
                                                <td>{index + 1}</td>
                                                <td>{item.odp_id}</td>
                                                <td>{item.type}</td>
                                                <td>{item.status}</td>
                                                <td>{item.odp_uplink}</td>
                                                <td>{item.odp_uplink_port}</td>
                                                <td>{item.kapasitas}</td>
                                                <td>{item.lokasi}</td>
                                                <td>{item.pon_id}</td>
                                                <td>{item.deskripsi}</td>
                                                <td>
                                                    <Link to={`/odp/edit/${item.odp_id}`}>
                                                        <button className='btn btn-sm btn-warning'>Edit</button>
                                                    </Link>
                                                    <Delete id={item.pon_id} onDataUpdate={setOdp} onPage={setPage} onLimit={setLimit} onRow={setRows} onTotalpage={setPages} />
                                                </td>
                                            </tr>
                                        </tbody>
                                    ) : <tbody>
